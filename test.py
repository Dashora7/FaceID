...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        # We can add background apprehension in loop but then that would require subject to move around after failure
        back = md.train_background(25)

        while 1:
            print('background found. Now waiting for motion')
            md.begin_motion_alerting(back)
            print('Motion found! Start Loop!')
            prevTime = 0
            tcount = 0
            HumanDict = {key: 0 for key in HumanNames}
            st = time.time()
            while time.time() - st < 10:
                    ...
                    if nrof_faces > 0:
                        ...
                        for i in range(nrof_faces):
                            ...
                            scaled.append(np.array(
                                Image.fromarray(cropped[0]).resize((image_size, image_size), resample=Image.BILINEAR)))
                ...
                # c+=1
                # NOT SHOWING FOR DEBUG RUNTIME ANALYZATION
                # cv2.imshow('Video', frame)

                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break

            # Check detection results to see if it all worked out
            for k, v in HumanDict.items():
                if v / tcount >= 0.8:
                    print('Detected as', k)
                    if k == 'unauthorized':
                        print('LOCKED')
